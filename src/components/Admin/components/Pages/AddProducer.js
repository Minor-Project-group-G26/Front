...
function AddProducer() {
    ...
    const ProSubmitHandler = async(e) =>{
        e.preventDefault();
        console.log(Pro)
        
        const newPro = new FormData();
        newPro.append('Name', Pro.pro);
        const res = await Axios({
            url: `http://127.0.0.1:5000/producer`, 
            data: newPro,
            method:'POST',
    ...
    return (
        <MyForm onSubmit={ProSubmitHandler}>
            <h1>Add New Producer</h1>
            <Myinput type="text" placeholder="Enter Producer Name" onChange={(e) => SetPro({...Pro, pro: e.target.value})} />
